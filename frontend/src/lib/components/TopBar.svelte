<script lang="ts">
	...
	let { username = '', isAdmin = false, onLogout, left }: Props = $props();
	...
	// Close dropdowns on navigation (when URL changes)
	$effect(() => {
	...
	function searchScore(text: string, searchQuery: string): number {
		...
		// Query starts with text (searching for partial title)
		...
		// Multi-word query: check if all query words appear in text
		...
		// Partial substring match (query is part of a word)
		for (const word of textWords) {
			if (word.includes(q)) return 60;
		}

		// Check if text words are in query (reverse partial match)
		for (const word of textWords) {
			if (word.length >= 3 && q.includes(word)) return 55;
		}

		// Character sequence matching (for typos/partial typing)
		let matchLen = 0;
		let tIdx = 0;
	...
	function handleSearchInput() {
		...
		debounceTimer = setTimeout(async () => {
			...
			try {
				...
				// Load library cache and TMDB results in parallel
				...
				// Parse TMDB results based on what we searched
				let tmdbMovies: any[] = [];
				let tmdbShows: any[] = [];
				let responseIdx = 0;
				...
				// Filter and score library results based on filter
				...
				// Build set of library TMDB IDs to exclude from discover results
				const libraryTmdbIds = new Set([
					...(cachedMovies || []).map((m: any) => m.tmdbId),
					...(cachedShows || []).map((s: any) => s.tmdbId),
				]);

				// Combine TMDB results, excluding items already in library
				// Sort by popularity (vote_count * rating approximation)
	...
	function navigateToResult(item: any) {
		...
		if (item.source === 'library') {
			if (item.type === 'movie') {
				...
				path = `/music/artists/${item.id}`;
			} else if (item.type === 'book') {
				path = `/books/${item.id}`;
			}
		} else {
			if (item.type === 'movie') {
				path = `/discover/movie/${item.id}`;
			} else if (item.type === 'show') {
				path = `/discover/show/${item.id}`;
			}
		}
...
<header class="fixed top-0 left-16 right-0 h-16 glass-topbar flex items-center justify-between px-6 z-30">
				...
				<div class="search-dropdown absolute left-0 right-0 top-full bg-[#111111] backdrop-blur-xl border border-white/10 !border-t-0 rounded-b-xl z-50 shadow-2xl">
					...
					<div class="flex items-center gap-1 p-2 border-b border-white/5">
						{#each filterOptions as option}
							<button
								...
								class="px-3 py-1.5 text-xs font-medium rounded-lg transition-all {searchFilter === option.id ? 'bg-white/15 text-white' : 'text-text-secondary hover:text-white hover:bg-white/5'}"
							>
								{option.label}
							</button>
						{/each}
					...
					<div class="search-results max-h-[60vh] overflow-y-auto">
						...
						{#if libraryResults.length > 0}
							<div class="p-2">
								...
								{#each libraryResults as item, i}
									...
									>
										<div class="w-10 h-14 bg-bg-card rounded overflow-hidden flex-shrink-0 {item.type === 'artist' ? '!rounded-full !w-10 !h-10' : ''}">
											{#if item.posterPath || item.imagePath || item.coverPath}
												<img
													src={`/api/images/poster${item.posterPath || item.imagePath || item.coverPath}`}
													alt=""
										...
										<div class="flex-1 text-left min-w-0">
											<p class="text-white text-sm font-medium truncate">{item.title || item.name}</p>
						...
						{#if discoverResults.length > 0}
							<div class="p-2 {libraryResults.length > 0 ? 'border-t border-white/5' : ''}">
								<p class="px-3 py-1.5 text-xs text-text-muted uppercase tracking-wider flex items-center gap-2">
								...
								{#each discoverResults as item, i}
									...
									<div
										...
										tabindex="0"
									>
										<div class="w-10 h-14 bg-bg-card rounded overflow-hidden flex-shrink-0">
											{#if item.poster_path}
												<img
										...
										{#if item.requested}
											<span class="text-xs text-white/60">Requested</span>
										{:else}
											<button
												onclick={(e) => handleInlineRequest(e, item)}
												class="liquid-btn-icon !w-8 !h-8 !rounded-full"
												title="Request"
											>
