<script lang="ts">
	...
	onMount(async () => {
		try {
			const playlist = await getSmartPlaylist(id);
			name = playlist.name;
			description = playlist.description || '';
			mediaType = playlist.mediaType as 'both' | 'movies' | 'shows';
			sortBy = playlist.sortBy as 'added' | 'title' | 'year' | 'rating' | 'runtime';
			sortOrder = playlist.sortOrder as 'asc' | 'desc';
			limitCount = playlist.limitCount || null;
...
<svelte:head>
	<title>Edit {name || 'Playlist'} - Outpost</title>
</svelte:head>
...
{:else}
	<div class="px-[60px] py-8 max-w-4xl">
		<div class="mb-8">
			<button onclick={() => goto(`/playlists/${id}`)} class="text-text-muted hover:text-text-primary transition-colors flex items-center gap-2 mb-4">
				<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
				</svg>
				Back to Playlist
			</button>
			<h1 class="text-3xl font-bold text-text-primary">Edit Playlist</h1>
			{#if isSystem}
				<p class="text-amber-400 mt-1 flex items-center gap-2">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
					</svg>
					This is a system playlist. Some settings may be restricted.
				</p>
			{/if}
		</div>
